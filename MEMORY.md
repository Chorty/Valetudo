...
2. TTS through Valetudo may play noise: its PATH (`/data/bin:/usr/local/bin:/usr/bin:/bin:...`) has no `ffmpeg` (it is `/data/vacuumstreamer/ffmpeg`), so conversion hits ENOENT and the MP3 is copied to `speech.wav` for `aplay`. Pre-existing (the old `which ffmpeg` path did the same); the bridge's `/say` uses the full path and is unaffected. Fix: pass the ffmpeg path through `ttsConfig`. Also, HA's `binary_sensor.valetudo_cleanusmaximus_speaking` never turned on while Valetudo's API reported `speaking: true` for ~7 s.
