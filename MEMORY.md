...
3. **Cleaning-latency re-test.** Real evidence since the CPU fix, both scenarios now captured on the same Foyer-only segment clean, same build (`a959c53f`), back to back on 2026-09-16:
   ...
   - **New finding, not yet explained:** video-on roughly *doubled* isolated p95 (+104.6%) and raised burst p95 27.2%, far more than the ~11.5%/~18.4% found in the July whole-house comparison. AVA and Valetudo CPU were nearly identical between the two runs (242.5%/241.6% and 16.1%/16.4%), so the extra latency isn't simple CPU contention -- `video_monitor` (10.4% CPU) and go2rtc (3.0%) were the only processes that changed. State, map and JS-bundle latencies all rose too, not just root, suggesting a shared bottleneck (network/bandwidth, not CPU) that a short, light workload like a single small room exposes more than a long, CPU-saturating whole-house clean does. Worth investigating before treating the July ~11.5% figure as representative.
   - Still needed for full confidence: a larger/whole-house comparison, and understanding the isolated-vs-burst-CPU disconnect above.
