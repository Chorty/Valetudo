...
- Active since 2026-09-15: Valetudo `7bddd63a` (plugin `ce3bf73`, MQTT "speaking" immediate-publish fix), SHA-256 `a6e3cfe8b2339a6df1e492a45ae705216641cf48971bebf435cea56b992ac7d4`, native unchanged (`1d0b187`); package `/Users/mattjoslin/Documents/ValetudoBackups/valetudo_speaking_20260915` (sealed, `DEPLOY_RECORD.txt`); rollback: `/data/valetudo.predeploy_speaking0915`
...
2. Fixed and deployed 2026-09-15, two rounds:
   - (`e5f95357`, plugin `475608f`, deploy ID `ttsfix0915`): Valetudo's process PATH had no `ffmpeg` (it is `/data/vacuumstreamer/ffmpeg`), so TTS conversion hit ENOENT and played the raw MP3 renamed `.wav` through `aplay`. `ttsConfig.ffmpegCommand` now carries the real path; new registration test locks it in. Verified by reading a live `/tmp/valetudo-tts-*/speech.wav` header mid-playback: genuine `RIFF...WAVEfmt` (was MP3 data before).
   - (`7bddd63a`, plugin `ce3bf73`, deploy ID `speaking0915`): HA's `binary_sensor.valetudo_cleanusmaximus_speaking` never turned on. Cause: it's a getter-only MQTT property, and `MqttController` only republishes those every 30s (`refreshInterval`); a spoken phrase (5-8s) almost always starts and ends between two polls. `TextToSpeechCapability` now emits a `speakingChanged` event on each real transition (`onSpeakingChanged`/`_setSpeaking`), and `TextToSpeechCapabilityMqttHandle` calls `.refresh()` on the "speaking" property immediately when it fires, the same explicit-refresh mechanism `PropertyMqttHandle` already uses for two-way (getter+setter) properties. Verified live: HA's sensor went `off -> on` at 0.42s after the request and back to `off` at 6.02s, matching real playback -- the first time it has ever been observed to turn on.
...
5. Done 2026-09-15 (camera login and HA re-check). `CAMERA_LOGIN=on` (conf copy `vacuumstreamer.conf.pre_login_20260915`): user `valetudo`, password in the repo `.env` as `VALETUDO_PASSWORD` (mode 600, git-excluded) and in `/data/vacuumstreamer/credentials`. The name overlaps the MCP server's Valetudo Basic Auth variable, so never load `.env` into the MCP server's environment. From the Mac, go2rtc API/RTSP give 401 without or with a wrong password and 200 with it; the robot itself is exempt. HA Generic camera `camera.192_168_1_31_2` has the credentials; `dashboard-cleaning` card uses `live_provider: ha` (microphone button removed, as agreed); stale `camera.192_168_1_31` references fixed in `dashboard-cleaning` and `dashboard-yard`. Verified through HA: still image 200 JPEG, HLS stream with fMP4 segments; video switch off paused the camera (go2rtc and video_monitor stopped) and on resumed it with the login still enforced; URL sensors present; TTS spoke (`aplay` with the path as one argument, API `speaking: true` ~7 s); mop drying, mop dock cleaning and auto-empty each started and returned to idle through HA buttons while docked. Undo the login: set `CAMERA_LOGIN=off` and `killall go2rtc`.
6. Capture user-started cleaning profiles (camera idle and watched) on `0a1c32f6` to re-test the cleaning-latency gate (#16). Docked baselines at matched uptime are done (2026-09-15, 24–45 min after the nightly reboot, `tools/profiles.sh nightly0915`): idle `2026-09-15T04-53-43-057Z_nightly0915-docked-camera-idle_kuEzHO` (isolated p95 58.1 ms), watched `2026-09-15T05-03-45-028Z_nightly0915-docked-camera-watched_YjXzQo` (74.5 ms), always `2026-09-15T05-13-53-060Z_nightly0915-docked-camera-always_PUbj01` (59.4 ms). Every gate passes against the 2026-07-25 baselines, zero HTTP failures, Valetudo CPU 3.4–3.5% (21–23% below July). Use these as the docked baselines from now on.
...
12. Done 2026-09-15/16. Investigated: the plugin was never actually linted at all -- backend's `lint` script (`eslint -c .automated.eslintrc.json .`) only applies `eslint:recommended` through override globs (`lib/**/*.js`, `index.js`, `util/**/*.js`) relative to `backend/`, which never match `vacuumstreamer-plugin/**`, and ESLint's `--config` flag is additive to the normal directory cascade rather than a scoping mechanism, so nothing here was ever checked by `npm run lint_all` or CI. Could not reproduce or confirm the "8 files with errors" claim under any config; the plugin is clean as of `eaf1551`. Gave the plugin its own `.eslintrc.json` (mirrors backend's explicit style rules plus `eslint:recommended`/`plugin:node/recommended`, applied to all its files rather than through backend's glob overrides) and wired `npm run lint_plugin` into `lint_all` and `nodejs.yml`, so `tools/build_valetudo.sh` and CI both cover it going forward.
...
15. Fixed 2026-09-16 (`4702b019`): `util/generate_build_metadata.js` read `.git/HEAD` by hand and only resolved the ref for `master`; any other branch stayed as the literal `"ref: refs/heads/<branch>"` string, and a worktree checkout (`.git` is a file, not a directory) failed outright and fell back to `"unknown"`. Now uses `git rev-parse HEAD`, which handles both correctly. Verified live in the `wt-parent` worktree on `feature/vacuumstreamer-switches`: previously would have been `"unknown"`, now resolves the real commit. Unmodified upstream code (last touched by upstream `436fae62`) -- worth offering upstream alongside item 1.
