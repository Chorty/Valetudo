...
3. **Cleaning-latency re-test -- video-priority cause found, fixed and verified; whole-house confirmation still open.** Same Foyer-only segment clean, same build (`a959c53f`), on 2026-09-16:
   - Video off: `root_isolated_ms` p95 155.6 ms, 0 HTTP failures (`/Users/mattjoslin/Documents/ValetudoProfiles/2026-09-17T01-20-34-352Z_foyer-cleaning-video-off_0RrWzC`, 96 samples). The user browsed the Valetudo GUI during this run, explaining 3 outlier samples (max 2034 ms) clustered at 01:12-01:18 UTC; p95 is a percentile and is largely unaffected by a few outliers.
   - Video on (an automated `ffmpeg` RTSP puller simulated a watched stream, not GUI browsing): `root_isolated_ms` p95 318.4 ms, 0 HTTP failures (`.../2026-09-17T01-45-10-211Z_foyer-cleaning-video-on_2WxMk1`, 96 samples).
   - Both pass the 500 ms absolute cleaning gate. Valetudo CPU averaged 16.1%/16.4% (video off/on) versus 45.8% in the pre-fix 2026-07-26 baseline -- the CPU fix is holding.
   - **Root cause found and fixed 2026-09-16.** The extra latency isn't CPU contention as usually measured (AVA/Valetudo CPU were nearly identical, 242.5%/241.6% and 16.1%/16.4%, between the two runs) -- it's scheduling priority. Valetudo deliberately self-lowers to nice 10 (`os.setPriority` in `Valetudo.js`, upstream, unmodified) so it never competes with AVA's real-time control loop, but `go2rtc`/`video_monitor` ran at the default nice 0, tied with AVA and *ahead of* Valetudo. On this 4-core SoC, cleaning alone pushes load past 14-20; at that point scheduling order decides who gets the CPU, and video was winning time away from Valetudo it was never meant to win -- explaining why every endpoint (state, map, JS bundle, root) got slower together, and why a lighter workload (one room, more spare capacity to lose) showed a much bigger relative hit than the CPU-saturated July whole-house runs.
   - Fix: native `vacuumstreamer` PR #4 first set `go2rtc`/`video_monitor` to `nice -n 10`, but that's *relative* to the caller's own niceness, not absolute -- go2rtc (parent: camera_supervisor.sh, nice 0) landed correctly at 10, but video_monitor is spawned through go2rtc (now nice 10) -> camera_wake.sh -> video_monitor_launch.sh, so its own `-n 10` added 10 more and landed at 19 (the clamp ceiling), caught by checking the live result on the robot rather than trusting the theory. PR #5 fixed it properly: `vs_exec_at_nice(TARGET, cmd...)` in `vacuumstreamer_lib.sh` computes the adjustment needed to reach an absolute target regardless of the calling chain. Both PRs merged to native `master` (`cd71f8b`).
   - Deployed to the robot 2026-09-16 as `go2rtc_launch.sh`, `video_monitor_launch.sh` and `vacuumstreamer_lib.sh` (deploy id `nicefix2_0916`; rollback: the matching `.predeploy_nicefix2_0916` files). Confirmed live through the real chain (restarted go2rtc, then woke video_monitor through an actual viewer connection): both now run at absolute nice 10, matching Valetudo instead of outranking it. Root/API 200, MQTT ready, watchdog stable, no errors.
   - **Verified live 2026-09-16 22:48 EDT.** A third Foyer clean, same build, same automated-viewer methodology, after the fix: `root_isolated_ms` p95 197.4 ms, 0 HTTP failures (`/Users/mattjoslin/Documents/ValetudoProfiles/2026-09-17T02-56-56-632Z_foyer-cleaning-video-on-nicefix_w7ERMV`, 96 samples). Down from the pre-fix 318.4 ms (-38.0%), now only +26.9% over the video-off baseline of 155.6 ms instead of +104.6% -- close to the July whole-house figure of ~11.5%, confirming the fix, not just the theory.
   - One unrelated outlier in this run: a single sample's isolated request took 4279.8 ms, with state (5184.6 ms), map (4772.5 ms) and the JS bundle (5701.9 ms) also very slow in the same concurrent batch -- but that same batch's `root_ms` was a normal 63 ms, and Valetudo's RSS showed no GC signature around it. Root ("/") is a ~829-byte static page needing almost no CPU; state/map/js all need real CPU to build (serialize state, read/compress the map, read the bundle). The likely explanation is Node's single-threaded event loop occasionally serializing that CPU-bound work under cleaning's concurrent load, not a priority/scheduling issue -- p95 is unaffected by one outlier, and it doesn't undercut the nice-priority fix. Worth a future look, lower priority, unrelated to this item.
   - Still needed for full confidence: a larger/whole-house comparison.
...
13. **The duststreamer experiment.** Explicitly gated behind a separate go-ahead and someone able to power-cycle the robot, since its driver can hard-lock the kernel. Kept last regardless of interest level, because of that risk and the explicit gate.
