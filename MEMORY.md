...
3. **Cleaning-latency re-test.** First real evidence since the CPU fix: a user-started Foyer-only clean on 2026-09-16 (`a959c53f`, video off/on-demand-idle) passed `root_isolated_ms` p95 155.6 ms against the 500 ms gate (`/Users/mattjoslin/Documents/ValetudoProfiles/2026-09-17T01-20-34-352Z_foyer-cleaning-video-off_0RrWzC`, 96 samples, 0 HTTP failures). Valetudo CPU averaged 16.1% versus 45.8% in the pre-fix 2026-07-26 baseline. Not yet a full pass: single small room rather than whole-house/multi-room, no video-on comparison run, and the user browsed the Valetudo GUI during the run, which explains 3 outlier samples (max 2034 ms) clustered at 01:12-01:18 UTC; p95 is a percentile and is largely unaffected by a few outliers, but this run should not be treated as a clean controlled baseline. Still needed: a video-on run, and ideally a larger/whole-house comparison for full confidence.
