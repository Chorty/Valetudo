...
- Active since 2026-09-15: Valetudo `e5f95357` (plugin `475608f`, TTS ffmpeg-path fix), SHA-256 `fc4a4230636afba56702330c31dc4ae7adf95d435f49b5b1e5345fadafc9929b`, native unchanged (`1d0b187`); package `/Users/mattjoslin/Documents/ValetudoBackups/valetudo_ttsfix_20260915` (sealed, `DEPLOY_RECORD.txt`); rollback: `/data/valetudo.predeploy_ttsfix0915`
- Previous: `0a1c32f6` (bridge allow list, plugin TTS-without-a-shell), SHA-256 `88d2e284a83a41293e2c51418f2accd577226ac7d1a338afe82bb9bf55e3e21a`, native `1d0b187`; package `/Users/mattjoslin/Documents/ValetudoBackups/valetudo_0a1c32f6_20260914`; rollback: every `*.predeploy_0a1c32f6` file (binary, `_root_postboot.sh`, `/data/vacuumstreamer/*`), then reboot
...
2. Fixed and deployed 2026-09-15 (`e5f95357`, plugin `475608f`, deploy ID `ttsfix0915`): Valetudo's process PATH had no `ffmpeg` (it is `/data/vacuumstreamer/ffmpeg`), so TTS conversion hit ENOENT and played the raw MP3 renamed `.wav` through `aplay`. `ttsConfig.ffmpegCommand` now carries the real path; new registration test locks it in. Verified by reading a live `/tmp/valetudo-tts-*/speech.wav` header mid-playback: genuine `RIFF...WAVEfmt` (was MP3 data before). Still open: HA's `binary_sensor.valetudo_cleanusmaximus_speaking` never turns on while Valetudo's own API reports `speaking: true` for several seconds -- a separate MQTT state-reporting gap.
