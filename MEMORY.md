...
3. **Cleaning-latency re-test.** Real evidence since the CPU fix, both scenarios now captured on the same Foyer-only segment clean, same build (`a959c53f`), back to back on 2026-09-16:
   ...
   - Still needed: a live re-verification (another Foyer video-on clean, or similar) to confirm this actually closes the latency gap, not just the mechanism; and a larger/whole-house comparison for full confidence.
