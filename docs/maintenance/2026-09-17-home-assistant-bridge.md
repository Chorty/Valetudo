...
| `rest_command.vacuum_set_video_quality`, `sensor.vacuum_video_quality` | User chose native replacements (2026-09-17). `RecorderQualityCapability` (REST plus a Home Assistant select entity) is deployed (2026-09-23). On 2026-09-24 the video quality dropdown was repointed: `Vacuum Video Quality Changed` calls `select.select_option` on `select.valetudo_cleanusmaximus_recorder_quality` only when the dropdown differs from it, `Vacuum Controls Sync on Startup` reads the native select, and a new `Vacuum Video Quality Sync` automation copies native changes into the dropdown. Verified end to end (dropdown high/low went through `recorder_quality_ctl.sh`, one `video_monitor` at nice 10). Backups: `/Users/mattjoslin/Documents/ValetudoBackups/ha_video_quality_repoint_20260924T185939Z`. `rest_command.vacuum_set_video_quality` and `sensor.vacuum_video_quality` are now unused and go with the bridge. **The `high` profile is broken (found 2026-09-24).** 640x480 makes `video_monitor` emit an 864x480 SPS while encoding 640-wide slices, so every decoder shows a green smear (errors start at macroblock column 40; reproduced on the raw encoder output, not go2rtc). The camera is fixed at 864x480 and ignores a 25 fps request (15 fps delivered). A test profile of 864x480/25/2 Mbps decoded with 0 errors at about double the low bitrate. Fix: change `high` in `recorder_quality_ctl.sh` (and the bridge handler) to 864x480 with the higher bitrate. Until then, leave the dropdown on `low`. |
